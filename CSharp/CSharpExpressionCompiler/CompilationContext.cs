...
namespace Microsoft.CodeAnalysis.CSharp.ExpressionEvaluator
{
    internal sealed class CompilationContext
    {
            ...
            DiagnosticBag diagnostics)
        {
            ...
            var allTypeParameters = ImmutableArray<TypeParameterSymbol>.Empty;
            ...
            if (!argumentsOnly && allTypeParameters.Length > 0)
            {
                ...
                typeVariablesType = new EENamedTypeSymbol(
                    ...
                    allTypeParameters,
                    (t1, t2) => allTypeParameters.SelectAsArray((tp, i, t) => (TypeParameterSymbol)new SimpleTypeParameterSymbol(t, i, _methodDebugInfo.Compiler.GetUnmangledTypeParameterName(tp.Name)), t2));
            ...
            var synthesizedType = new EENamedTypeSymbol(
                ...
                (m, container) =>
                {
                    ...
                    if (!argumentsOnly)
                    {
                        ...
                        if ((!m.IsStatic && !IsDisplayClassType(m.ContainingType, _methodDebugInfo.Compiler)) || GetThisProxy(_displayClassVariables) != null)
                        {
                            var methodName = GetNextMethodName(methodBuilder);
                            var method = this.GetThisMethod(container, methodName);
                            localBuilder.Add(new CSharpLocalAndMethod("this", "this", method, DkmClrCompilationResultFlags.None, LocalAndMethodKind.This, 0)); // Note: writable in dev11.
        ...
        private static LocalAndMethod MakeLocalAndMethod(LocalSymbol local, MethodSymbol method, DkmClrCompilationResultFlags flags)
        {
            ...
            return new CSharpLocalAndMethod(escapedName, displayName, method, flags, (local as EELocalSymbolBase)?.LocalAndMethodKind ?? LocalAndMethodKind.Local, (local as EELocalSymbolBase)?.Index ?? -1);
        }

        private EEAssemblyBuilder CreateModuleBuilder(
            CSharpCompilation compilation,
            ...
            DiagnosticBag diagnostics)
        {
            ...
            return new EEAssemblyBuilder(
                ...
                additionalTypes,
                contextType => GetNonDisplayClassContainer(((EENamedTypeSymbol)contextType).SubstitutedSourceType, _methodDebugInfo.Compiler),
                testData);
        }
            ...
            out ImmutableArray<LocalSymbol> declaredLocals)
        {
            ...
            binder = new EEMethodBinder(compiler, method, substitutedSourceMethod, binder);
        ...
        /// </summary>
        private ImmutableArray<LocalSymbol> GetLocalsForBinding(
            ImmutableArray<LocalSymbol> locals,
            ...
            ImmutableDictionary<string, DisplayClassVariable> displayClassVariables)
        {
            ...
            foreach (var local in locals)
            {
                ...
                if (_methodDebugInfo.Compiler.GetKind(name) != GeneratedNameKind.None)
                {
                    continue;
                }
                ...
                if (_methodDebugInfo.Compiler.IsSynthesizedLocalName(name))
                {
                    continue;
                }
        ...
        private ImmutableArray<string> GetSourceMethodParametersInOrder(
            MethodSymbol method,
            MethodSymbol? sourceMethod)
        {
            var containingType = method.ContainingType;
            bool isIteratorOrAsyncMethod = IsDisplayClassType(containingType, _methodDebugInfo.Compiler) &&
                                           _methodDebugInfo.Compiler.GetKind(containingType.Name) == GeneratedNameKind.StateMachineType;
            ...
            {
                Debug.Assert(IsDisplayClassType(containingType, _methodDebugInfo.Compiler));
                foreach (var member in containingType.GetMembers())
                {
                    ...
                    if (_methodDebugInfo.Compiler.GetKind(fieldName) == GeneratedNameKind.None)
                    {
            ...
            else
            {
                if (sourceMethod is object)
                {
                    foreach (var p in sourceMethod.Parameters)
                    {
                        ...
                        if (_methodDebugInfo.Compiler.GetKind(parameterName) == GeneratedNameKind.None &&
                            !IsDisplayClassParameter(p, _methodDebugInfo.Compiler))
                        {
            ...
            out ImmutableDictionary<string, DisplayClassVariable> displayClassVariables)
        {
            ...
            {
                if (_methodDebugInfo.Compiler.GetKind(parameter.Name) == GeneratedNameKind.TransparentIdentifier ||
                    IsDisplayClassParameter(parameter, _methodDebugInfo.Compiler))
                {
                    var instance = new DisplayClassInstanceFromParameter(_methodDebugInfo.Compiler, parameter);
                    displayClassInstancesOutside.Add(new DisplayClassInstanceAndFields(_methodDebugInfo.Compiler, instance));
                }
            }

            if (IsDisplayClassType(currentFrame.ContainingType, _methodDebugInfo.Compiler) && !currentFrame.IsStatic)
            {
                // Add "this" display class instance.
                var instance = new DisplayClassInstanceFromParameter(_methodDebugInfo.Compiler, currentFrame.ThisParameter);
                displayClassInstancesOutside.Add(new DisplayClassInstanceAndFields(_methodDebugInfo.Compiler, instance));
            }
            ...
            {
                var name = local.Name;
                if ((name != null) && (_methodDebugInfo.Compiler.GetKind(name) == GeneratedNameKind.DisplayClassLocalOrField))
                {
                    ...
                    if (localType is object && displayClassTypes.Add(localType))
                    {
                        ...
                        displayClassInstancesInside.Add(new DisplayClassInstanceAndFields(_methodDebugInfo.Compiler, instance));
                    }
                }
            }
            ...
            buildResult(displayClassInstancesInside, inScopeHoistedLocalSlots, parametersAreOutside ? ImmutableArray<string>.Empty : parameterNamesInOrder, hoistedVarFieldTokenToNamesMap, displayClassVariablesBuilder, displayClassVariableNamesInsideInOrderBuilder);
            ...
            if (!currentFrame.IsStatic && isPrimaryConstructor)
            {
                checkForPrimaryConstructor = !tryAddCapturedPrimaryConstructorParameters(currentFrame, shadowingParameterNames: ImmutableArray<string>.Empty,
                                                                                         ...
                                                                                         displayClassVariablesBuilder, displayClassVariableNamesInsideInOrderBuilder, _methodDebugInfo.Compiler);
            }

            buildResult(displayClassInstancesOutside, inScopeHoistedLocalSlots, parametersAreOutside ? parameterNamesInOrder : ImmutableArray<string>.Empty, hoistedVarFieldTokenToNamesMap, displayClassVariablesBuilder, displayClassVariableNamesOutsideInOrderBuilder);
            ...
            if (!isPrimaryConstructor && checkForPrimaryConstructor && currentFrame == currentSourceMethod && !currentFrame.IsStatic)
            {
                checkForPrimaryConstructor = !tryAddCapturedPrimaryConstructorParameters(currentFrame, shadowingParameterNames: parameterNamesInOrder,
                                                                                         possiblyCapturingType: currentFrame.ContainingType,
                                                                                         possiblyCapturingTypeInstance: (Instance: null, Fields: ConsList<FieldSymbol>.Empty),
                                                                                         displayClassVariablesBuilder, displayClassVariableNamesOutsideInOrderBuilder, _methodDebugInfo.Compiler);
            }
                    ...
                    if (!field.IsStatic && GeneratedNameParser.TryParsePrimaryConstructorParameterFieldName(field.Name, out string? parameterName))
                    {
                            ...
                            !shadowingParameterNames.Contains(parameterName))
                        {
                            if (possiblyCapturingTypeInstance.Instance is null)
                            {
                                ...
                                possiblyCapturingTypeInstance.Instance = new DisplayClassInstanceFromParameter(compiler, currentFrame.ThisParameter);
                            }

                            DisplayClassVariable variable = new DisplayClassVariable(compiler, parameterName, DisplayClassVariableKind.Parameter,
        ...
        private void GetAdditionalDisplayClassInstances(
        ...
        private void GetDisplayClassInstances(
            HashSet<TypeSymbol> displayClassTypes,
            ArrayBuilder<DisplayClassInstanceAndFields> displayClassInstances,
            DisplayClassInstanceAndFields instance)
        {
            ...
            foreach (var member in instance.Type.GetMembers())
            {
                ...
                var field = (FieldSymbol)member;
                if (!_methodDebugInfo.Compiler.IsDisplayClassInstance(field.Type.Name, field.Name) && !IsTransparentIdentifierField(field))
                {
                    continue;
                }
                ...
                {
                    var other = instance.FromField(_methodDebugInfo.Compiler, field);
                    displayClassInstances.Add(other);
                }
            }
        }

        bool IsTransparentIdentifierField(FieldSymbol field)
        {
            ...
            case GeneratedNameKind.AnonymousTypeField:
                if (_methodDebugInfo.Compiler.GetKind(part) != GeneratedNameKind.TransparentIdentifier)
                {
                    return false;
                }
                break;
        ...
        /// </summary>
        private static bool IsDisplayClassParameter(ParameterSymbol parameter, CompilerKind compiler)
        {
            var type = parameter.Type;
            var result = type.Kind == SymbolKind.NamedType && IsDisplayClassType((NamedTypeSymbol)type, compiler);
...
REPARSE:
                ...
                switch (fieldKind)
                {
                    ...
                    case GeneratedNameKind.HoistedLocalField:
                        ...
                        if (_methodDebugInfo.Compiler.TryParseSlotIndex(fieldName, out int slotIndex) &&
                ...
                if (displayClassVariablesBuilder.TryGetValue(variableName, out var displayClassVariable))
                {
                    ...
                    if (!instance.Fields.Any())
                    {
                        if (variableKind == DisplayClassVariableKind.Local)
                        {
                            ...
                            Debug.Assert(instance.Instance is DisplayClassInstanceFromLocal ||
                                         (instance.Instance is DisplayClassInstanceFromParameter && _methodDebugInfo.Compiler.GetKind(instance.Type.Name) == GeneratedNameKind.LambdaDisplayClass));
                        }
                        else
                        {
                            Debug.Assert(variableKind == DisplayClassVariableKind.Parameter);
                            Debug.Assert(_methodDebugInfo.Compiler.GetKind(instance.Type.Name) == GeneratedNameKind.StateMachineType);

                            if (variableKind == DisplayClassVariableKind.Parameter && _methodDebugInfo.Compiler.GetKind(instance.Type.Name) == GeneratedNameKind.StateMachineType)
                            {
                    ...
                    else
                    {
                        ...
                        if (variableKind == DisplayClassVariableKind.Parameter && _methodDebugInfo.Compiler.GetKind(instance.Type.Name) == GeneratedNameKind.LambdaDisplayClass)
                        {
                            displayClassVariablesBuilder[variableName] = instance.ToVariable(variableName, variableKind, field);
                        }
                    }
                }
                else if (variableKind != DisplayClassVariableKind.This || _methodDebugInfo.Compiler.GetKind(instance.Type.ContainingType.Name) != GeneratedNameKind.LambdaDisplayClass)
                {
        ...
        private static bool IsDisplayClassType(TypeSymbol type, CompilerKind compiler)
        {
            if (type.Kind == SymbolKind.NamedType)
            {
                switch (compiler.GetKind(type.Name))
                {
        ...
        private static NamedTypeSymbol GetNonDisplayClassContainer(NamedTypeSymbol type, CompilerKind compiler)
        {
            ...
            bool sourceMethodMustBeInstance)
        {
                ...
                compiler.TryParseSourceMethodNameFromGeneratedName(candidateSubstitutedSourceMethod.Name, GeneratedNameKind.LocalFunction, out desiredMethodName))
            {
                ...
                var substitutedSourceType = GetNonDisplayClassContainer(candidateSubstitutedSourceType, compiler);
        ...
        private readonly struct DisplayClassInstanceAndFields
        {
            ...
            internal DisplayClassInstanceAndFields(CompilerKind compiler, DisplayClassInstance instance) :
                this(compiler, instance, ConsList<FieldSymbol>.Empty)
            {
                Debug.Assert(IsDisplayClassType(instance.Type, compiler) ||
            ...
            private DisplayClassInstanceAndFields(CompilerKind compiler, DisplayClassInstance instance, ConsList<FieldSymbol> fields)
            {
            ...
            internal DisplayClassInstanceAndFields FromField(CompilerKind compiler, FieldSymbol field)
            {
                Debug.Assert(IsDisplayClassType(field.Type, compiler) ||
                             compiler.GetKind(field.Type.Name) == GeneratedNameKind.AnonymousType);
                return new DisplayClassInstanceAndFields(compiler, this.Instance, this.Fields.Prepend(field));
            }

            internal DisplayClassVariable ToVariable(string name, DisplayClassVariableKind kind, FieldSymbol field)
            {
                return new DisplayClassVariable(compiler, name, kind, this.Instance, this.Fields.Prepend(field));
            }
